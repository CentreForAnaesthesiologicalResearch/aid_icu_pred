...
ggplot_age_mortality <- df |>
  ggplot(aes(age)) +
  geom_density(aes(fill = mort_1y), alpha = 0.4) +
  viridis::scale_fill_viridis(discrete = T) +
  scale_y_continuous(labels = percent_format()) +
  theme_classic() +
  labs(
...
ggplot_hematological_cancer_mortality <- df |>
  count(hematologic_cancer, mort_1y) |>
...
ggplot_metastatic_cancer_mortality <- df |>
  count(metastatic_cancer, mort_1y) |>
...
ggplot_emergency_surgery_mortality <- df |>
  count(emergency_surgery, mort_1y) |>
...
ggplot_elective_surgery_mortality <- df |>
  count(elective_surgery, mort_1y) |>
  mutate(pct = n / sum(n)) |>
...
# Correlations ------------------------------------------------------------
df_num <- df |>
  dplyr::select(c(mort_1y, sex, age, alcohol_abuse, strata_delir, advanced_cancer, surgery, sms, icu_type, pre_barthel, pre_cps, pre_cfs, limitation, intervention, emergency_surgery, elective_surgery, hematologic_cancer, metastatic_cancer, smoking, barthel_category, cfs_category, cps_category)) |>
